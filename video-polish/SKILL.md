...
拍摄环境不好看时，**优先做景深虚化，不要换背景**。

```bash
pip install mediapipe        # 只有这个功能需要，模型 250KB 自动缓存
python3 $VP/vp_bg.py 原片.mp4 --blur 30 --sharpen 0.6 --out 虚化.mp4
```

虚化之所以是首选：背景是真实房间、真实光线、真实颗粒，只是失焦，看不出任何后期痕迹。
再把主体锐化一点，"浅景深"的对比就立起来了，观感直接接近正经机器拍的。

换成一张合成背景则很难不假——光比、透视、噪点、色温任何一项对不上，边缘立刻露馅。
**只有原背景实在没法看（墙上有杂物、私密信息）才用**：
...
用 `--bg` 时务必让背景亮度接近原素材：亮的人贴在暗背景上，边缘一眼就看出是抠的。
自带的三种（`bookshelf` / `studio` / `grid`）是矢量排版，**不是照片**；要照片级效果，
用即梦、Midjourney 生成一张再喂给 `--bg`。

调参：

| 参数 | 默认 | 作用 |
|------|------|------|
| `--blur` | — | 虚化半径。720 宽用 22~30，1080 宽用 34~45 |
| `--sharpen` | 0.55 | 主体锐化。源片软就加大，超过 1.2 会有毛边 |
| `--dim` / `--desat` | 0.12 / 0.18 | 背景压暗、降饱和，让主体更突出 |
| `--erode` | 0.06 | 向内收边，去掉原背景残留的亮边。有白边就加大 |
| `--feather` | 3 | 边缘羽化。太小像剪纸，太大人会发虚 |
| `--smooth` | 0.55 | 掩码时间平滑。边缘抖就加大，动作拖影就减小 |

速度约 4 帧/秒（4 核 CPU），21 秒素材约 2.5 分钟。

**做不到的事**，别硬试：换脸、改体型、往手里加物体、凭空加一台电脑——这些要生成模型，
这套流程里没有。让用户直接实拍摆进画面，或用即梦 / 可灵这类工具单独做。
...
- **背景虚化后人物边缘有光晕**：脚本已经用归一化卷积避免了人物颜色糊出去；
  仍有光晕说明掩码收边不够，加大 `--erode`。
